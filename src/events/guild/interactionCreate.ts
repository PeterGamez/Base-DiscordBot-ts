...
export default async (client: Client, interaction: ChatInputCommandInteraction | ButtonInteraction | ModalSubmitInteraction | AnySelectMenuInteraction) => {
    if (interaction.isChatInputCommand()) {
        ...
        console.log(`[${new Date().toString().split(" ", 5).join(" ")}] runSlash | (${interaction.guild.id} | ${interaction.user.id}) | ${interaction.commandName} ${interaction.options.data.map((x) => x.value).join(" ")}`)
